...
a) This MAC procedure is not a good one. It is possible to recover K from the original message M and MAC(K,M). An adversary needs to only intercept MAC(K,M) and the message M to begin decrypting the MAC using AES. 
...
In reverse, we would again pad M and break it into n pieces. Then apply the decryption of AES as follows:
